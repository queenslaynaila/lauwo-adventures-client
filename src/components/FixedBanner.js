...
const FixedBanner = () => {
  ...
  {banners.length > 0 && banners.map((banner) => (
    <div key={banner.id} className="absolute lg:top-[18%]  top-[10%] right-0 left-0  flex flex-col   w-full">
      <div className="bg-yellow-400 text-white p-4 flex justify-between items-center">
        <p className="text-lg">{banner.message}</p>
        <button onClick={handleCloseBanner} className="text-white text-2xl">
          <IoClose />
        </button>
      </div>
    </div>

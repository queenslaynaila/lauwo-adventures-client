...
export default function Mountains({ mountains }) {
  ...
  const handlePrevClick = () => {
    setCurrentImageIndex((prevIndex) =>
      prevIndex === 0 ? mountains.length - 1 : prevIndex - 1
    );
  };
  const handleNextClick = () => {
    setCurrentImageIndex((prevIndex) =>
      prevIndex === mountains.length - 1 ? 0 : prevIndex + 1
    );
  };
  return (
    <>
      <Head>
        ...
        <meta
          ...
          content="Explore the beauty of mountains"
        />
        <meta
          property="og:image"
          content={mountains[currentImageIndex].image_url}
        />
        <meta
          property="og:url"
          content={`https://example.com/mountain-trekking/${generateSlug(
            mountains[currentImageIndex].mountain_name
          )}`}
        />

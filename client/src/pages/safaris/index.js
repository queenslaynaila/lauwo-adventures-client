    ...
    >
      ...
      <div className="flex flex-col items-center justify-center h-full w-full ">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-24 mb-12 p-4">
          {safaris.map((safari) => (
            <Link className="p-4" key={safari.id} href={`/safaris/${generateSlug (safari.name)}`}>
              <div className="border-2 border-white relative shadow-lg
              ...
              ">
               ...
               <div className="bg-black opacity-40 absolute inset-0"></div>
                <div className="p-4 w-full h-full text-white  flex justify-center items-center absolute top-6">
                  <h3 className="text-md font-bold uppercase font-poly z-3 text-center">{safari.name}</h3>
                </div>
                </div>
            </Link>
          ))}

...
export default function Packages({ packages }) {
  const { standard_inclusive, standard_exclusive, premium_inclusive, premium_exclusive } = packages;

  const standardInclusiveItems = standard_inclusive ? standard_inclusive.split(",") : [];
  const standardExclusiveItems = standard_exclusive ? standard_exclusive.split(",") : [];
  const premiumInclusiveItems = premium_inclusive ? premium_inclusive.split(",") : [];
  const premiumExclusiveItems = premium_exclusive ? premium_exclusive.split(",") : [];
  ...
  return (
    <div>
      <div className="bg-white py-4 text-center text-3xl font-bold uppercase mt-2">PACKAGES</div>
      <div className="bg-sand px-8 py-12">
       
      <div className="py-2 text-center text-2xl font-bold uppercase ">STANDARD</div>
        <table className="w-full mx-auto border-collapse text-black">
          <thead>
            <tr style={{ backgroundColor: "#FFF4D2" }}>
              <th className="bg-background py-4 px-8 font-bold uppercase">Inclusive</th>
              <th className="bg-background py-4 px-8 font-bold uppercase">Exclusive</th>
            </tr>
          </thead>
          <tbody>{standardTableRows}</tbody>
        </table>
        <div className="py-2 text-center text-2xl font-bold uppercase ">PREMIUM</div>
        <table className="w-full mx-auto border-collapse text-black">
          <thead>
            <tr style={{ backgroundColor: "#FFF4D2" }}>
              <th className="bg-background py-4 px-8 font-bold uppercase">Inclusive</th>

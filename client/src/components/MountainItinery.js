...
export default function MountainItinery({itinery,index}) {
  ...
  return (
    ...
    <div className={`flex justify-center ${index % 2 === 0 ? "bg-sand" : "bg-light"}`}>
     
      <div className={`w-1/2 m-4 relative ${index % 2 !== 0 ? "order-last" : ""}`}>
        <div className="absolute top-0 left-0   w-16 h-16 text-white text-6xl font-bold">{itinery.day_no}</div>
